 ...
 * Displays a chronological horizontal stepper showing the active status
 * of the AI generation pipeline (Routing -> Researching -> Orchestrating -> Drafting -> Finalizing).
...
interface LiveStatusIndicatorProps {
  currentStatus: string; // Active state: 'routing', 'researching', 'orchestrating', 'writing', 'finalizing', 'complete', 'error', 'pending'
  message: string;        // Log string representing detail explanation of current status
}

export function LiveStatusIndicator({ currentStatus, message }: LiveStatusIndicatorProps) {
  // Ordered step configuration map
  const steps = [
  ...
  // Helper to identify the index of the active step in the array
  ...
  return (
    <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 shadow-xl my-6">
      <div className="flex items-center justify-between mb-8 relative">
        {/* Decorative background pipeline link */}
      ...
      {/* Detailed log status message block */}
